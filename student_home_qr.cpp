...
Student_Home_Qr::Student_Home_Qr(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::Student_Home_Qr)
{
    ui->setupUi(this);
    ...
    Student_Home_Qr::selectInfoFromDatabase($logKey_studentId);
    DateTimeUtils::updateDateTimeUtils(ui->dateLabel, ui->timeLabel);

    // Connect ui objects to functions based on user interaction
    connect(ui->backButton, &QPushButton::clicked, this, &Student_Home_Qr::switchWindow_StudentLoginForm);
    connect(ui->authCodeButton, &QPushButton::clicked, this, &Student_Home_Qr::switchWindow_StudentHomeAuth);
...
void Student_Home_Qr::selectInfoFromDatabase(const QString &key_studentId)
{
    ...
    Student_Home_Qr::displayInfoFromDatabase(studentDataList);
    Student_Home_Qr::displayDataFromDatabase(studentDataList);
    Student_Home_Qr::generateQr(studentDataList);
}


void Student_Home_Qr::displayInfoFromDatabase(const QStringList &dataList)
{
...
void Student_Home_Qr::displayDataFromDatabase(const QStringList &dataList)
{
    // Assign members from dataList to variables
    QString studentId = dataList[0];
    QString lastName = dataList[2];
...
void Student_Home_Qr::generateQr(const QStringList &dataList)
{
    // Assign members from dataList to variables
    QString studentId = dataList[0];
    QString lastName = dataList[2];
    ...
    QString qrData = QString("%1@%2@%3@WVSU@%4").arg(studentId, lastName, college, authCode);
...
void Student_Home_Qr::switchWindow_StudentLoginForm()
{
...
void Student_Home_Qr::switchWindow_StudentHomeAuth()
{
    // Switch ui window to Student_Home_Auth
    student_home_auth = new Student_Home_Auth;
    student_home_auth->show();
    this->hide();
}


void Student_Home_Qr::moveEvent(QMoveEvent *event)
{
