    ...
    , ui(new Ui::Lecturer_Home)
{
    ...
    Lecturer_Home::selectInfoFromDatabase($logKey_lecturerId);
    DateTimeUtils::updateDateTimeUtils(ui->dateLabel, ui->timeLabel);

    // Connect ui objects to functions based on user interaction
    connect(ui->backButton, &QPushButton::clicked, this, &Lecturer_Home::switchWindow_LecturerLoginForm);
    connect(ui->classListButton, &QPushButton::clicked, this, &Lecturer_Home::switchWindow_LecturerClassesList);
...
void Lecturer_Home::selectInfoFromDatabase(const QString &key_lecturerId)
{
    ...
    Lecturer_Home::displayInfoFromDatabase(lecturerDataList);
    Lecturer_Home::displayDataFromDatabase(lecturerDataList);
    Lecturer_Home::generateQr(lecturerDataList);
}


void Lecturer_Home::displayInfoFromDatabase(const QStringList &dataList)
{
...
void Lecturer_Home::displayDataFromDatabase(const QStringList &dataList)
{
    // Assign members from dataList to variables
    QString lastName = dataList[2];
...
void Lecturer_Home::generateQr(const QStringList &dataList)
{
    // Assign members from dataList to variables
    ...
    QString qrData = QString("%1@%2@%3@WVSU").arg(lecturerId, lastName, college);
...
void Lecturer_Home::switchWindow_LecturerLoginForm()
{
...
void Lecturer_Home::switchWindow_LecturerClassesList()
{
...
void Lecturer_Home::moveEvent(QMoveEvent *event)
{
